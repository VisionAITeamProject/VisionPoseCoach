...
from network.ble_gatt_server import BLEBackendUnavailable, BlueZGattServer, DEVICE_NAME
...
async def run(args):
    ...
    server = BlueZGattServer(manager, device_name=args.device_name)
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            ...
            pass
    await server.start()
    print(f"BLE advertising: {args.device_name} (WiFi mode: {wifi.mode})")
    try:
